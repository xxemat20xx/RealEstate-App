...
const AdminPanel = () => {
  ...
  const handleAddAgent = async (agentData) => {
    try {
      ...
      setIsAddingAgent(false);  // Close the modal after save
      setEditingAgent(null);  // Clear the editing state
      await getAgents();  // Refresh agent list
    } catch (error) {
        ...
        {view === 'overview' ? (
                          ...
                          <div className="flex items-center gap-6">
                            <div className="relative">
                              <img
                                ...
                                className="max-w-16 min-h-16 rounded-2xl object-cover shadow-lg border border-slate-100"
                              />
                        ...
                        <td className="px-8 py-6 text-right">
                          <div className="flex justify-end gap-3 opacity-40 group-hover:opacity-100 transition-opacity">
                            ...
                            >
                              ...
                              {showAlert && (
                                <Alert
                                  message={`Are you sure you want to delete ${property.title.toUpperCase()}?`}
                                  onConfirm={() => {
          ...
          <>
            <div className="max-w-[1400px] mx-auto">
              <div className="flex justify-between items-end mb-10">
                <div>
                  <h2 className="text-3xl font-serif font-bold text-slate-900">Agent Overview</h2>
                  <p className="text-slate-500 mt-2">Manage {agents.length} active agents.</p>
                </div>
                <button
                ...
                <table className="w-full text-left table-auto">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {agents.map((agent) => (
                      ...
                      {showAlert && (
                        <Alert
                          message={`Are you sure you want to delete ${agent.name.toUpperCase()}?`}
                          onConfirm={() => {
      ...
      {isAddingAgent && (
        <AddAgentForm
          ...
          onCancel={() => {
            ...
            setEditingAgent(null);     // Reset editing agent state
          }}
          agent={editingAgent}  // Pass agent to fill the form when editing
        />
      )}
